...
const HowItWorks = () => {
  ...
  return (
    <section ref={containerRef} className="py-24 relative overflow-hidden bg-gradient-to-b from-white to-slate-50">
      {/* City skyline silhouette in background */}
      <div className="absolute bottom-0 inset-x-0 h-32 opacity-[0.02] bg-[radial-gradient(#000_1.5px,transparent_1.5px)] bg-[size:16px_16px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-6 bg-slate-950 transform -translate-x-1/2 rounded-full border border-white/5 shadow-2xl overflow-hidden">
            {/* Dashed Road Lanes */}
            <div className="w-full h-full border-l border-dashed border-tascki-yellow/40 mx-auto w-0.5 opacity-80" />
            
            {/* Highlighted active route road with glowing shadow */}
            <motion.div 
              style={{ height: taxiY }}
              className="absolute top-0 left-0 right-0 bg-tascki-yellow/30 shadow-[0_0_12px_rgba(255,192,0,0.5)] pointer-events-none"
            />
          </div>
          ...
          <motion.div 
            style={{ top: taxiY }}
            className="hidden md:flex absolute left-1/2 -translate-x-1/2 -mt-4 w-9 h-9 rounded-full bg-tascki-yellow border-2 border-slate-950 items-center justify-center shadow-[0_4px_15px_rgba(255,192,0,0.6)] z-20 cursor-pointer"
          >
            <Car size={16} className="text-slate-950 rotate-180 stroke-[2.5]" />
          </motion.div>
              ...
              <div key={index} className={`flex flex-col md:flex-row items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="flex-1 w-full md:px-12 py-4">
                  <motion.div 
                    ...
                    className={`bg-white p-8 rounded-[2rem] border border-slate-200/60 shadow-lg shadow-slate-100/50 relative hover:shadow-2xl hover:border-amber-400/40 transition-all ${
                      index % 2 === 0 ? 'md:text-left text-left' : 'md:text-right text-left'
                    }`}
                  >
                    <span className="text-tascki-yellow text-7xl font-black opacity-[0.09] absolute top-4 right-6 pointer-events-none select-none font-mono">
                      {step.num}
                    </span>
                    
                    <h3 className="text-2xl font-black text-slate-950 mb-2 relative z-10 tracking-wide uppercase">
                      {step.title}
                    </h3>
                ...
                <div className="hidden md:flex justify-center items-center w-12 h-12 relative z-10">
                  <div className="w-5 h-5 bg-slate-950 rounded-full border-4 border-tascki-yellow shadow-[0_0_8px_#ffc000]" />
                </div>
                
                {/* Spacer side column */}
                <div className="hidden md:block flex-1 w-full md:px-12" />
              </div>
            ))}
          </div>
        </div>
